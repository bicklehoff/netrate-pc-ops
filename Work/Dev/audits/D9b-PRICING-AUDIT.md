...
| E4 | ~~`src/lib/rates/homepage-db.js`~~ → `src/lib/rates/homepage.js` | **RETIRED 2026-04-17 (D9b.3)** — old file deleted. New `homepage.js` is a thin wrapper around `priceScenario()` + `pickParRate()`, no duplicated plumbing. Historical: previously reimplemented orchestration, own SQL, own broker-config, own filters. | thin wrapper on canonical | `src/app/page.js`, `src/app/rate-watch/page.js` |
| E5 | `src/lib/rates/pick-par-rate.js` | Pure par-rate selector — "lowest rate with finalPrice ≥ 100, else closest-to-par." | **CANONICAL** (selection) | homepage.js, RateTool hook, (should be but isn't) E2 |
| E6 | `src/lib/rates/sub-financing-comparison.js` | **PARALLEL** — inlines a hardcoded GSE LLPA grid (`src/lib/rates/sub-financing-comparison.js:18-42`) and hardcoded `COMP_RATE = 0.02`, `COMP_CAP_REFI = 3595` (`:47-48`). Used for the HELOC/Cash-out comparison math. | **parallel — rewrite, not delete** | **LIVE:** `src/components/calculators/SubFinancingComparison.js:11` → `src/app/tools/second-lien-comparison/page.js:12` (public page returns 200). **Correction:** original audit said "no grep hits" — verified 2026-04-17 to be incorrect. Absence claims in this doc must be reverified per the "never assume" rule. |
...
- **[CRITICAL C3] Rewrite `sub-financing-comparison.js`.** Hardcoded FNMA/FHLMC LLPA grid (`:18-42`) — stale the day it was written. Hardcoded `COMP_RATE=0.02, COMP_CAP_REFI=3595` (`:47-48`). **Verified 2026-04-17: this is LIVE code** (powers `/tools/second-lien-comparison`, prod 200) — do not delete. **Fix:** rewrite `compareSubFinancingOptions` to call `priceScenario()` twice (keep-second vs pay-off) and diff. Scope: medium.
