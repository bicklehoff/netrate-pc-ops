...
| D9d · Reference data migration | County limits, closing costs, PLF tables, comp caps → DB tables. D8 inventory findings feed this. | 🟡 **spec shipped + first table live.** Umbrella design at [`D9d-REFERENCE-DATA-SPEC.md`](./D9d-REFERENCE-DATA-SPEC.md) defines the 13 `ref_*` tables + universal schema pattern (temporal versioning, source citation, global scope). **Migration 020 (#___)** shipped `ref_conforming_baselines` (1 row) + `ref_county_loan_limits` (3,235 counties seeded from `src/data/county-loan-limits.js`) + DAL at `src/lib/rates/ref-loan-limits.js` with 1-hour cache + temporal `asOf` support. Consumer retirement scoped to a follow-up PR. Unblocks D9b.10. |
