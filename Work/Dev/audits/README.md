...
| D9b · Pricing unification | One API entry point, product router, retire homepage-db.js parallel path, all surfaces use same engine | 🟡 **8 of 13 shipped.** D9b.3 retire homepage-db (#107), D9b.4 par-rule unification (#112), D9b.5 site_scenarios + D9b.6 surface_pricing_config (#114), D9b.7 homepage_rate_cache (#115), **D9b.9 pricer-integrated tool defaults (#___)** — purchase-calculator, refi-analyzer, cost-of-waiting now SSR the default rate from `homepage_rate_cache` via 2-file pattern (`page.js` server + `content.js` client), falling back to old hardcoded literal on cache miss; 10-min ISR. **Remaining: D9b.8** sub-financing-comparison.js rewrite (done via #109 tool rewrite — verify), **D9b.10** FHA_BASELINE_LIMIT derivation (pending D9d), **D9b.11** state closing-cost map → DB table, **D9b.13** duplicate getFicoBand in engine.js (>=800) vs pricing-v2.js (>=780). |
