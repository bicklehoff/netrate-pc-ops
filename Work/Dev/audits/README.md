...
| D9d · Reference data migration | County limits, closing costs, PLF tables → DB tables. D8 inventory findings feed this. | 🟡 **3 of 13 tables live.** Umbrella design at [`D9d-REFERENCE-DATA-SPEC.md`](./D9d-REFERENCE-DATA-SPEC.md) defines the 13 `ref_*` tables + universal schema pattern (temporal versioning, source citation, global scope). **Migration 020** shipped `ref_conforming_baselines` (1 row) + `ref_county_loan_limits` (3,235 counties) + DAL at `src/lib/rates/ref-loan-limits.js`. **Migration 022 (#148)** shipped `ref_fha_ufmip` (4 rows, HUD ML 2015-01 seed) + DAL at `src/lib/rates/ref-fha-ufmip.js`. **Migration 023 (#149)** shipped `ref_va_funding_fee` (empty schema — seed deferred per spec §8 Q2 until VA pricing is real). Consumer retirement queued as follow-up PRs. **Spec corrections (2026-04-22):** (1) `ref_comp_caps` was proposed in Pass 2 HP-B6 as a D9d table, but comp caps are NetRate broker policy — not externally-authored per §2 inclusion rule — and already live in `rate_lenders`. **`ref_comp_caps` removed from the D9d table list.** (2) EOD 2026-04-21 referred to `ref_hecm_config` as shorthand; the spec §4.3 defines five separate HECM tables (`_limits`, `_ufmip`, `_annual_mip`, `_plf`, `_pricing`). Next-session HECM work picks one table, not a consolidated `_config`. |
