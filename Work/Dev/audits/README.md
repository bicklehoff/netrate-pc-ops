...
**Current driver:** PC Dev (`nice-heisenberg-e5d8b9` worktree)
**Last updated:** 2026-04-24 (v1.21 — D9c UAD amendments AD-10a/11a/12a shipped #197; D6 #34 scope pivot from Everstream Core NonQM parser to ResiCentral DSCR second-lender expansion. See §11 change log.)
...
- **Outstanding (revised 2026-04-24):**
  - ~~Core Non-QM LLPA parser (DSCR Plus + Expanded Prime Plus core tier adjustments) — follow-up to PR 15c.~~ **Superseded 2026-04-24.** Everstream Core Non-QM sheet uses a different layout requiring significant parser work. Alternative strategy: add ResiCentral as a second DSCR lender (cleaner sheet, 5 DSCR products vs Everstream's 3 Elite tiers, already David's best-priced DSCR lender). See item §7/#34 for the revised plan.
...
| D9b · Pricing unification | One API entry point, product router, retire homepage-db.js parallel path, all surfaces use same engine | ✅ **complete — all assigned items shipped** (.1/.2/.12 numbering slots were never assigned to real work; .3–.11 and .13 cover the full intended scope). D9b.11 (state closing-cost map → DB) shipped in #___ via migration 024 (`ref_state_closing_costs`, 4-state seed) + DAL + parity-check script, same "DAL is truth, static file is client mirror" pattern as D9b.10. D9b.3 retire homepage-db (#107), D9b.4 par-rule unification (#112), D9b.5 site_scenarios + D9b.6 surface_pricing_config (#114), D9b.7 homepage_rate_cache (#115), **D9b.8 sub-financing-comparison** verified closed via #109 — `src/components/calculators/SubFinancingComparison.js` is standalone (user enters rates, pure math, no pricer calls, no invented LLPA adjustments). **D9b.9 pricer-integrated tool defaults** (#130) — purchase-calculator, refi-analyzer, cost-of-waiting now SSR the default rate from `homepage_rate_cache` via 2-file pattern; 10-min ISR. **D9b.10 FHA_BASELINE_LIMIT derivation** (#___) — closed on "DAL is truth, static file is a build-time mirror" grounds. Server pricing reads `ref_conforming_baselines` via `getConformingBaseline()`; `defaults.js` no longer exports `FHA_BASELINE_LIMIT`; `fee-builder.js` takes `highBalanceThreshold` as a DB-sourced param. The client-bundled `src/data/county-loan-limits.js` remains as a synchronous UX mirror (county dropdowns, high-cost badges) — drift is prevented by `scripts/check-loan-limits-parity.mjs`. **D9b.13 getFicoBand de-dup** (#___) — clarified that the `engine.js:getFicoBand` (UI display, fine-grained `>=800`/`780-799`) and `pricing-v2.js` band utility (LLPA lookup key, coarse `>=780` bucket) serve intentionally different purposes. Renamed pricing-v2's private function to `ficoBandKey` to eliminate the name collision; added JSDoc on both + `db-adj-loader.js` explaining the distinction so they never get merged. **Remaining: D9b.11** state closing-cost map → DB table (blocked on new `ref_state_closing_costs` table — overlaps D9d scope). |
| D9c · Scenario/quote model | Scenarios link to Contact + Deal, composable calc modules, Layer 3 Lite (shareable quote links), borrower scenario save | 📋 spec drafted + **3 amendments 2026-04-24 (AD-10a/11a/12a in [UAD-SPEC](../UAD-SPEC.md#2-architecture-decisions))** — queued after D9b wrap. Pre-work: (1) split `scenarios` into scenarios/rate_alerts/quotes, (2) concrete calculator module contract with version pinning + render variants, (3) snapshot-on-send immutability for quote links. |
| D9d · Reference data migration | County limits, closing costs, PLF tables → DB tables. D8 inventory findings feed this. | ✅ **non-HECM scope complete** (11 tables live; `ref_fee_templates` rename dropped; `ref_licensed_states` formalized). Remaining spec items all land in the standalone HECM project (`ref_hecm_pricing`, `ref_lender_corrections`) or are externally-blocked (`ref_fha_annual_mip` pending case_number_date tracking). Umbrella design at [`D9d-REFERENCE-DATA-SPEC.md`](./D9d-REFERENCE-DATA-SPEC.md) defines the 13 `ref_*` tables + universal schema pattern (temporal versioning, source citation, global scope). **Migration 020** shipped `ref_conforming_baselines` (1 row) + `ref_county_loan_limits` (3,235 counties) + DAL at `src/lib/rates/ref-loan-limits.js`. **Migration 022 (#148)** shipped `ref_fha_ufmip` (4 rows, HUD ML 2015-01 seed) + DAL at `src/lib/rates/ref-fha-ufmip.js`. **Migration 023 (#149)** shipped `ref_va_funding_fee` (empty schema — seed deferred per spec §8 Q2 until VA pricing is real). **Migration 024 (#153)** shipped `ref_state_closing_costs` (4-state seed: CO, TX, OR, CA) + DAL at `src/lib/rates/ref-state-closing-costs.js` + parity check at `scripts/check-state-closing-costs-parity.mjs` — closed D9b.11. **Migration 025 (#154)** shipped three HECM scalar tables bundled — `ref_hecm_limits` (1-row MCA per HUD), `ref_hecm_ufmip` (0.02 rate per HUD ML 2017-12), `ref_hecm_annual_mip` (0.005 rate per HUD ML 2017-12) + combined DAL at `src/lib/rates/ref-hecm-scalars.js`. **Migration 026 (#155)** shipped `ref_hecm_plf` — the full HUD Principal Limit Factor grid (128 expected rates × 38 ages = 4,864 cells, seeded from `src/lib/hecm/plf-table.js`, HUD ML 2017-12 effective date) + DAL at `src/lib/rates/ref-hecm-plf.js`. **HECM remainder declared its own project (2026-04-22)** — `ref_hecm_pricing` (rate-sheet pipeline) + HECM consumer retirement + FOA `ref_lender_corrections` are scoped into a standalone HECM project, not absorbed into the D9d throughput batch. **Migration 027 (#___)** shipped two tables bundled: `ref_state_tax_rates` (51 rows — 50 states + DC from Census Bureau / Tax Foundation estimates) + `ref_county_tax_rates` (empty schema — populated as county-assessor data lands) + combined DAL at `src/lib/rates/ref-tax-rates.js` with county→state fallback + parity check. **Drift finding (deferred):** `src/components/RefinanceCalc/shared.js` has its own 4-state tax rates that diverge from the Census Bureau values in the DB (e.g. CA 0.75% vs 0.71%). Reconciling that drift is a separate D9d follow-up — not conflated with this migration. Consumer retirement queued as follow-up PRs. **Spec corrections (2026-04-22):** (1) `ref_comp_caps` was proposed in Pass 2 HP-B6 as a D9d table, but comp caps are NetRate broker policy — not externally-authored per §2 inclusion rule — and already live in `rate_lenders`. **`ref_comp_caps` removed from the D9d table list.** (2) EOD 2026-04-21 referred to `ref_hecm_config` as shorthand; the spec §4.3 defines five separate HECM tables (`_limits`, `_ufmip`, `_annual_mip`, `_plf`, `_pricing`). Next-session HECM work picks one table, not a consolidated `_config`. |
| D9e · Application modules | Clean segment model for deal data (borrower identity, employment, assets, property, loan, co-borrower, declarations), clone deal, Application LITE | ✅ **Phase 1 complete** (PRs #180–#190 — `loan_borrowers` retired; all 17 src/ files rewired to `loan_participants` + satellites; migration 048 dropped the table + 3 satellite FK cols; 13 ADs captured (D9e-1–D9e-13)). Phase 2 (`loans.*` denorm columns, ~622 rows) is a separate future project. |

### D10 · MLO Portal Exercise Audit — 🆕 filed (2026-04-22)

Complement to D7 (surface-scoped): D10 is **user-flow scoped** — exercising the portal end-to-end and catching bugs that only surface during real use. First findings surfaced during the 2026-04-21 Payam Kavousi incident. Audit doc: [`D10-MLO-PORTAL-EXERCISE-AUDIT-2026-04-22.md`](./D10-MLO-PORTAL-EXERCISE-AUDIT-2026-04-22.md).

- **D10.1** `/api/portal/mlo/contacts` 500 — Critical, live breakage. Root cause: raw `sql(text, [params])` call-style not supported by current `@neondatabase/serverless`. **Shipped** — converted to `sql.query(text, params).rows` per working pattern in `scenarios/db.js`.
- **D10.2** Pipeline columns silently hidden — High, live UX bug. Root cause: saved `netrate_pipeline_state` predates recent column additions; no schema-version marker. **Shipped** — schema-versioned fix (persist `knownColumns` snapshot, diff on load, add new `defaultVisible` columns without un-hiding explicit user hides). Naive auto-merge rejected.
- **D10.3** Prequal letter discoverability — Medium, UX debate. Deferred to D7 IA redesign track (IA-2–IA-7).
...
### Done 2026-04-24 (D9e Phase 1 — `loan_borrowers` retirement)

33. ✅ **D9e Phase 1 — application modules schema + code rewire** (PRs #180–#190) — Foundation (#180): 6 migrations (032–037) + Prisma removal + DB cleanup. Plan (#181): 571-line `Work/Dev/UAD-LAYER-D9E-PLAN.md`, 13 architecture decisions locked (D9e-1 through D9e-13). Phase 1 schema (#182–#188): `loan_housing_history`, `loan_staff_assignments`, contacts column additions, soft-delete on 4 top-level tables, `loan_demographics` satellite, satellite FK rewire (contact_id on employments + declarations), `marital_status` added to `loan_participants`. Code rewire (#182–#189): all 17 src/ files rewired off `loan_borrowers` → `loan_participants` + satellites (response key `loan_borrowers` preserved in GET responses — data source changed, shape-dependent consumers required zero changes). Drop (#190): migration 048 dropped `loan_borrowers` + 3 `loan_borrower_id` FK columns from satellites; payroll/route.js trailing INSERT fixed atomically. Site 200 OK post-drop verified.

### Next up (punch list reconciled 2026-04-24 after D9e Phase 1 closure)

**Medium — scope-bounded, session-sized:**

34. **D6 ResiCentral DSCR — second-lender expansion** (revised 2026-04-24, supersedes prior "Core NonQM LLPA parser" scope). Original plan was to parse Everstream's Core Non-QM sheet for DSCR Plus + Expanded Prime Plus core tier adjustments. Revised after inspecting ResiCentral's rate sheet (`67370_04242026_1128199760.xlsx`): ResiCentral has **5 DSCR products** (Premier, Investor Premier, Elite, Elite FN, Select) — broader coverage than Everstream Elite's 3 tiers — with a meaningfully cleaner sheet layout (standard Rate × Lock-Days grid + FICO×LTV LLPA grid matching the Everstream Elite shape). ResiCentral is David's best-priced DSCR lender today and the broker relationship is active. Pivot the scope:
    - **Short-term (this item):** Parse ResiCentral's 5 DSCR rate-sheet tabs + LLPA tabs. Extend `nonqm_rate_sheets` + `nonqm_rate_products` to accept `lender_code='resicentral'`. Upgrade DSCR pricer to multi-lender comparison (currently single-lender on Everstream) — mirrors the forward pricer's multi-lender pattern.
    - **Drop:** Everstream Core Non-QM sheet parsing. The `isCore` short-circuit in `src/lib/pricing-nonqm/price-dscr.js:191` stays but the warning changes from "core_llpas_not_loaded" to "not_pursued_in_favor_of_resicentral" (or similar). ResiCentral covers the product gap with cleaner pricing.
    - **Follow-on backlog items** (filed in PLATFORM-2026, not in this audit queue): ResiCentral forward suite (Conv/FHA/VA/Jumbo, 20+ tabs — makes ResiCentral a 7th forward lender), ResiCentral NQM + Bank Statement Elite (unlocks bank statement as a new `scenarios.loan_type` surface). Both are multi-session efforts downstream of this item.
    - Scope for this item: 1–2 sessions. Parser + ingest + pricer multi-lender upgrade + DSCR calculator regression test.

**Architectural — session-of-its-own:**

35. **D9c scenario/quote model** — composable calc modules, shareable quote links (Layer 3 Lite), borrower scenario save. Multi-PR effort.
36. **D9e Phase 2** (future project) — retire `loans.*` denormalized borrower columns (`monthly_base_income`, `employment_status`, `employer_name`, `current_address`, etc.) for the ~622 settled loans that carry data there, then drop those columns from the `loans` table. Neon-branch rehearsal mandatory.
37. **HECM standalone project** (scoped out of D9d 2026-04-22) — `ref_hecm_pricing` rate-sheet pipeline + HECM consumer retirement + FOA `ref_lender_corrections`. Blocks HECM optimizer retheme in item 44 below.
38. **D7 nav/IA redesign (IA-2 through IA-7 + D10.3 Prequal discoverability)** — dedicated consultation session with David. Not a self-service PR.

**D7 small residuals (bundle as convenient):**

39. **LE-3 product call** — Run Quote vs Generate Quote merge — product decision needed first.
40. **Quote list/detail retheme** — `src/app/portal/mlo/quotes/page.js` still on cyan/teal palette.
41. **`borrower_* → contact_*` API rename** — pipeline + loans/[id] routes. 48 references across 9 files (QuoteWizard, QuoteScenarioForm, QuotePDF, HecmOptimizer, PayrollSection). Tier A with quote-generation smoke test. UAD follow-through — the schema already says `contact_id` post-D9a; the API surface is lagging.
42. **STATUS_LABELS harmonization** — `loan-states.js` (has `rate_alert`) vs `loan-statuses.js` (has `settled`/`withdrawn`). Different keys. Design call needed: which map wins, where does `rate_alert` live.
43. **`docs_out` teal-500 token** — `loan-statuses.js:57` hardcodes `bg-teal-500 text-white`. Trivial extract to a const if doing other work in the file.
44. **HECM optimizer retheme (9 files)** — blocked on HECM standalone project; do not retheme throwaway code.

**Data quality (post-vocab follow-ups surfaced in re-audit):**

45. **Column-name unification** — `loans.loan_term` vs `scenarios.term` vs `rate_products.term`. Values are canonical (years) but column name diverges. Trivial rename migration when convenient.
46. **`loans.refi_purpose` NULL gap** — 464 refi rows have null `refi_purpose` (only 68 of 532 refis are classified). Not a vocab defect; a data-completeness issue from historical imports. Needs classification strategy call.
47. **`loans.loan_term` non-standard values** — 24 rows have 12 / 19 / 28 / 29 year terms. Likely remaining-term values from historical imports (documented as a known issue in the vocab audit §3.5).

**Externally blocked:**

48. **`ref_fha_annual_mip`** — pending `case_number_date` tracking implementation in loan schema (D9d spec §8).

**Needs David input:**

49. **MKT-B3 volume numbers** — needs fresh 2026-YTD numbers OR continues as 2025-labeled historical stats.
50. **Claw backlog items** — #86 related-articles widget, #87 Ahrefs re-crawl verification, #88 DSCR screening workflow, plus pre-existing #76 /refinance + #79 sequence engine.

**Cross-device pickup (outside FoH scope):**

51. **Mac relay `relay_moam5f5e8zxys9tb` part 2** — pdf-tools signing queue wiring + migration + UI warning badge. Mac shipped part 1 (PR #48, commit 58da6a7) with the `strip-form-layer` + `audit-pdf` library. PC owes the signing queue intake/completion gates + `form_audit jsonb` schema addition.

**Ops cleanup (non-code):**

52. **Delete 2 test/spam leads from DB** — `fast@example.com` (smoke test from D1b) + the `MRQSUwflQBHOiAGjNgGND-style` scraper submission that slipped through before the honeypot landed.
...
- **2026-04-24 (v1.21)** — **D9c pre-work amendments + D6 #34 scope pivot.** Two shifts filed same-day. (1) UAD-SPEC AD-10a/11a/12a amendments shipped via #197 — scenarios table split into scenarios/rate_alerts/quotes, concrete calculator module contract with version pinning, snapshot-on-send immutability for quote links. All three amend D9c scope; none revise D9a/D9b/D9d/D9e. Architecture evaluated under "will it endure, will it scale" lens and three load-bearing concerns documented before D9c build begins. (2) Item #34 scope revised from "parse Everstream Core Non-QM sheet" to "add ResiCentral as second DSCR lender" after inspecting ResiCentral's rate sheet. ResiCentral covers 5 DSCR products (vs Everstream's 3 Elite tiers) with a cleaner sheet layout + active broker relationship + best-priced DSCR offering. Everstream Core Non-QM sheet parsing is dropped in favor of ResiCentral integration. Three follow-on backlog items filed in PLATFORM-2026: `RESICENTRAL-DSCR-PARSER` (this #34 work), `RESICENTRAL-FORWARD-SUITE` (Conv/FHA/VA/Jumbo 7th-lender expansion), `RESICENTRAL-NONQM-BANKSTATEMENT` (unlocks bank statement as new scenarios.loan_type). D6 "Outstanding" bullet rewritten to reflect the pivot.
- **2026-04-24 (v1.20)** — **D9e Phase 1 complete.** `loan_borrowers` fully retired: schema foundation (#180, migrations 032–037 + Prisma removal), D9e plan (#181, 571-line `UAD-LAYER-D9E-PLAN.md` + 13 ADs D9e-1–D9e-13), Phase 1 build (#182–#189, new satellites + 17 src/ files rewired), drop (#190, migration 048 + payroll/route.js fix). Response shape preserved — `loan_borrowers` key in GET responses unchanged, shape-dependent consumers required zero changes. Site 200 OK post-drop verified. **Header:** `D9c/e queued` → `D9e ✅ Phase 1 complete (PRs #180–#190); D9c queued`. **D9e dimension row** updated to Phase 1 complete; Phase 2 (`loans.*` denorm columns) noted as future project. **Queue:** item 33 (Done 2026-04-24) filed for D9e Phase 1; old item 35 (D9e application modules) replaced with D9e Phase 2 (item 36); old item 36 (HECM) HECM cross-ref corrected from item 42 → 44; all Next up items renumbered 34–52.
...
- **2026-04-22 (v1.17)** — **D9d non-HECM scope declared complete.** Two spec closeouts, docs-only: (1) `ref_fee_templates` rename **dropped** — `fee_templates` stays as-is; it's a fee-template catalog with operational fields (`id uuid`, `status`, `label`, 25 per-fee columns, 12 rows), not pure reference data per the §2 inclusion test. Renaming a live operational catalog for cosmetic `ref_*` prefix consistency = real blast radius (new table + 12-row copy + 3 SELECT updates in `src/lib/quotes/fee-builder.js` + deploy + drop-old soak) for zero correctness benefit. (2) `ref_licensed_states` **formalized** — the ref table built during D7-3 picklist consolidation (PR #122, migration 012) counts as the D9d licensed-states reference with no rebuild; consumers continue through `src/lib/picklists/db-loader.js` + `/api/picklists`. **D9d dimension now** ✅ **complete for the non-HECM scope** — 11 tables live, 1 dropped, 1 formalized pre-existing, 3 (all HECM-related) handed off to the standalone HECM project, 1 (`ref_fha_annual_mip`) externally-blocked.
- **2026-04-22 (v1.16)** — **D9d tax tables + HECM scope recut.** Migration 027 bundled `ref_state_tax_rates` (51-row Census Bureau seed) + `ref_county_tax_rates` (empty schema per spec §4.4; county-assessor data lands as it's sourced). DAL at `src/lib/rates/ref-tax-rates.js` with transparent county→state-avg fallback. Parity check `scripts/check-state-tax-rates-parity.mjs` verifies the 51-state seed against `src/data/county-tax-rates.js` STATE_AVERAGES. Drift noted between DB and `src/components/RefinanceCalc/shared.js` (4 states with divergent rates) — filed as a D9d follow-up finding, not reconciled here. **HECM scope recut:** the remaining HECM work (`ref_hecm_pricing` + consumer retirement + FOA corrections) is a standalone "HECM from ground up" project (David, 2026-04-22), not absorbed into D9d throughput. D9d 9/13 → 11/13 tables live.
- **2026-04-22 (v1.15)** — **D9d ref_hecm_plf live (4,864-cell grid).** Migration 026 seeded the full HUD PLF lookup table — 128 expected rates (3.000% to 18.875% in 0.125% increments) × 38 ages (62-99) — from `src/lib/hecm/plf-table.js`. Regex-parsed from the ESM source (same pattern as `check-state-closing-costs-parity.mjs`). Seeded in 10 batches of 500; spot-check confirmed (6.500%/age 74 → PLF 0.4110 matches JS file comment). Rate stored as bps integer for exact-match indexing. DAL at `src/lib/rates/ref-hecm-plf.js` with percent→bps conversion and throws-on-missing contract. No consumer wired yet. D9d 8/13 → 9/13 tables live.
- **2026-04-22 (v1.14)** — **D9d 3 HECM scalar tables live.** Migration 025 bundled `ref_hecm_limits` + `ref_hecm_ufmip` + `ref_hecm_annual_mip` into one PR since all three are single-row scalars sharing the same cadence (HUD mortgagee letters + FHA annual MCA). Seeds mirror `src/lib/hecm/constants.js` exactly — no behavior change. Combined DAL at `src/lib/rates/ref-hecm-scalars.js` exposes `getHecmMaxClaimAmount`, `getHecmUfmip`, `getHecmAnnualMip`. HECM consumer retirement (constants.js → DAL across rate-sheet.js + plf-table.js) queued for follow-up. D9d count now 8/13 tables live (3 new in this PR; also corrects a prior off-by-one in the dimension counter — `ref_va_funding_fee` schema-only from migration 023 had been omitted). `ref_hecm_plf` (~4,800 rows) + `ref_hecm_pricing` (rate-sheet pipeline) + `ref_lender_corrections` (optional) remain in the HECM cluster.
- **2026-04-22 (v1.13)** — **D9b complete + D9d 4th table live.** Migration 024 shipped `ref_state_closing_costs` (4-state seed) + DAL + parity check — closes D9b.11 on the same "DAL is truth, static file is client mirror" pattern as D9b.10. D9b dimension now ✅ complete: .3–.11 and .13 cover the full intended scope; .1/.2/.12 were numbering placeholders never assigned to real work. D9d 3/13 → 4/13 tables live.
- **2026-04-22 (v1.12)** — **D10 filed + D9d fha_ufmip consumer retirement.** D10 MLO Portal Exercise Audit added as a new dimension, complementing surface-scoped D7 with user-flow triage. Initial 3 findings filed from the 2026-04-21 Payam incident; 2 shipped (D10.1 Contacts 500 fix, D10.2 Pipeline columns schema-versioned fix), 1 deferred to D7 IA redesign (D10.3 Prequal nav discoverability). Separately, `ref_fha_ufmip` consumer retirement shipped in PR #151 — `pricing-v2.js` + `fee-builder.js` now read FHA UFMIP rate from the DAL; `FHA_UFMIP_RATE` constant in `src/lib/constants/fha.js` retained only as a client display mirror for `QuoteScenarioForm`.
- **2026-04-22 (v1.11)** — **D9d 2 new tables live + DEV-PLAYBOOK rehearsal criteria sharpened.** Migration 022 shipped `ref_fha_ufmip` (4-row HUD ML 2015-01 seed, DAL, no consumer retirement yet — follow-up). Migration 023 shipped `ref_va_funding_fee` as empty schema per spec §8 Q2 (populate when VA pricing is real). D9d 1/13 → 3/13 tables live. DEV-PLAYBOOK.md updated with a matrix distinguishing "rehearse mandatory" (UPDATE/DELETE on existing data, multi-step transforms) from "rehearse skip" (new-table additive migrations, empty-schema tables, CREATE INDEX, ADD COLUMN). Principle: rehearse when the blast radius includes live rows.
- **2026-04-22 (v1.10)** — **D9b.10 closed + D9d spec corrections.** D9b.10 (FHA_BASELINE_LIMIT derivation) closed on "DAL is truth, static file is a build-time mirror" grounds — server pricing code already reads from `ref_conforming_baselines` via `getConformingBaseline()`; `src/data/county-loan-limits.js` stays bundled client-side for synchronous UX (county dropdowns, high-cost badges on /rates + portal quote form) with drift prevention via new `scripts/check-loan-limits-parity.mjs`. D9d spec corrections: `ref_comp_caps` removed (fails §2 externally-authored test — NetRate broker policy, already in `rate_lenders`); EOD "hecm_config" shorthand clarified against the spec's 5-HECM-tables layout. D9b status updated 10/13 → 11/13.
