...
**Status:** Active · Dimensions 1–5 done · 6 in progress · 7 pending · 8 new (inventory phase)
...
### D3 · Pricing engine correctness — ✅ done
...
- **Pass 2 · Homepage + UI components** — ⏳ not started. Seed findings from tonight:
  - `src/app/page.js:56-58` — hardcoded `'5.875%' / '5.94%' / '$2,366'` fallbacks for conv30/conv15/fha30/va30 hero rates. Silent borrower-facing stale-data fallback.
...
### Right now (week of Apr 15)

1. **File this spec** (this PR).
2. **Verify production state of PR #73 (homepage par picker)** — `ship immediately` category. Diagnose whether the 5.875% production display is the new picker firing or the `page.js:56` hardcoded fallback. Outcome informs whether PR #73 stays, is reverted, or is refactored.
3. **D8 inventory Pass 2 · Homepage + UI components** — likely produces one `ship-immediately` remediation (`page.js:56-58` hardcoded fallbacks) that can go right after the inventory doc. DSCR widget par-picker extraction is a `batch` candidate — hold for the consumer audit.
4. **D8 inventory Passes 3–4** — may run in parallel with Pass 2 (different surfaces):
