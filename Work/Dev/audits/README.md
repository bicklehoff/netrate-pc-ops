...
| D9d · Reference data migration | County limits, closing costs, PLF tables → DB tables. D8 inventory findings feed this. | 🟡 **8 of 13 tables live.** Umbrella design at [`D9d-REFERENCE-DATA-SPEC.md`](./D9d-REFERENCE-DATA-SPEC.md) defines the 13 `ref_*` tables + universal schema pattern (temporal versioning, source citation, global scope). **Migration 020** shipped `ref_conforming_baselines` (1 row) + `ref_county_loan_limits` (3,235 counties) + DAL at `src/lib/rates/ref-loan-limits.js`. **Migration 022 (#148)** shipped `ref_fha_ufmip` (4 rows, HUD ML 2015-01 seed) + DAL at `src/lib/rates/ref-fha-ufmip.js`. **Migration 023 (#149)** shipped `ref_va_funding_fee` (empty schema — seed deferred per spec §8 Q2 until VA pricing is real). **Migration 024 (#153)** shipped `ref_state_closing_costs` (4-state seed: CO, TX, OR, CA) + DAL at `src/lib/rates/ref-state-closing-costs.js` + parity check at `scripts/check-state-closing-costs-parity.mjs` — closed D9b.11. **Migration 025 (#___)** shipped three HECM scalar tables bundled — `ref_hecm_limits` (1-row MCA per HUD), `ref_hecm_ufmip` (0.02 rate per HUD ML 2017-12), `ref_hecm_annual_mip` (0.005 rate per HUD ML 2017-12) + combined DAL at `src/lib/rates/ref-hecm-scalars.js`. HECM consumer retirement (`src/lib/hecm/constants.js` → DAL) queued as a follow-up. Consumer retirement queued as follow-up PRs. **Spec corrections (2026-04-22):** (1) `ref_comp_caps` was proposed in Pass 2 HP-B6 as a D9d table, but comp caps are NetRate broker policy — not externally-authored per §2 inclusion rule — and already live in `rate_lenders`. **`ref_comp_caps` removed from the D9d table list.** (2) EOD 2026-04-21 referred to `ref_hecm_config` as shorthand; the spec §4.3 defines five separate HECM tables (`_limits`, `_ufmip`, `_annual_mip`, `_plf`, `_pricing`). Next-session HECM work picks one table, not a consolidated `_config`. |
