...
export async function POST(request, { params }) {
  ...
  const ticketRows = await sql`SELECT id FROM tickets WHERE id = ${id} AND organization_id = ${orgId} LIMIT 1`;
  ...
  const entryRows = await sql`
    ...
    VALUES (
      ...
      ${mloId}, ${`${session.user.firstName} ${session.user.lastName}`},
      'comment', NOW()
    )
    RETURNING *
  `;
  ...
  await sql`UPDATE tickets SET updated_at = NOW() WHERE id = ${id} AND organization_id = ${orgId}`;
