...
export default function LoanSidebar({ loanId, loanStatus }) {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <nav className="hidden lg:flex flex-col w-56 bg-surface-alt/50 border-r border-gray-200 h-full flex-shrink-0 overflow-y-auto">
        <div className="py-4 flex flex-col h-full">
          {SECTIONS.map((group) => (
            <div key={group.group} className="mb-2">
              <span className="px-4 text-[10px] font-bold text-ink-subtle uppercase tracking-widest">
                {group.group}
              </span>
              <div className="mt-1">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-4 py-2 text-sm transition-colors ${
                        isActive
                          ? 'bg-white text-brand font-medium border-r-2 border-brand shadow-nr-sm'
          ...
          <div className="mt-auto pt-4 border-t border-gray-200">
            <button
              ...
              className="w-full flex items-center gap-2 px-4 py-2 text-sm text-ink-subtle hover:text-ink-mid hover:bg-white/60 transition-colors"
            >
