  ...
  return (
    <div className={className}>
      <div className="relative">
        ...
        <label
          htmlFor={name}
          className="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-ink-subtletransition-all pointer-events-none
      ...
      {helper && !error && (
        <p className="text-xs text-ink-subtlemt-1">{helper}</p>
      )}
      {error && (
    ...
    <div>
      <div className="relative">
        ...
        <button
          ...
          className="absolute right-3 top-1/2 -translate-y-1/2 text-ink-subtlehover:text-ink-mid transition-colors"
      ...
      <p className="text-xs text-ink-subtlemt-1">
  ...
  return (
    <div className={className}>
      <div className="relative">
        ...
        <label
          htmlFor={name}
          className="absolute left-4 top-2 text-[11px] text-ink-subtlepointer-events-none"
        >
          {label}{required && ' *'}
        </label>
        {/* Chevron */}
        <svg className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-ink-subtlepointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
export function CurrencyField({
  ...
  if (setValue && watch) {
    ...
    return (
      <div className={className}>
        <label htmlFor={name} className="block text-sm font-medium text-ink-mid mb-1">
          {label} {required && <span className="text-red-400">*</span>}
        </label>
        <div className="relative">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-ink-subtletext-sm">$</span>
          <input
            id={name}
            type="text"
            inputMode="decimal"
...
export function AddressGroup({ prefix, register, errors, label = 'Address', disabled = false, setValue }) {
  ...
  const disabledClass = disabled ? 'bg-surface-alt text-ink-subtle cursor-not-allowed' : '';

  return (
    <fieldset>
      <legend className="text-sm font-medium text-ink-mid mb-2">{label}</legend>
