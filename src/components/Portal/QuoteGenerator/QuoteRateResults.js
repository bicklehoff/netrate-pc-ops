...
export default function QuoteRateResults({ pricing, selectedRates, onSelectRates, onReprice, loading, onNext, borrowerPaid, escrowsWaived, scenario }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
      ...
      <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onReprice}
            disabled={loading}
            className="px-3 py-1.5 text-xs font-medium text-brand bg-brand-light rounded-lg hover:bg-brand-light disabled:opacity-50"
          >
      ...
      {scenario?.loan_type === 'fha' && results[0]?.ufmip > 0 && (
                  ...
                  <div className="flex items-center gap-4 ml-4">
                    ...
                    <button
                      ...
                      className={`px-3 py-1 text-xs font-medium rounded-lg transition-colors ${
                        isSelected(par || best)
                          ? 'bg-brand text-white'
                          : 'bg-gray-100 text-gray-700 hover:bg-brand-light hover:text-brand-dark'
                      }`}
                    >
                        ...
                        {g.rates.map((r, i) => {
                          ...
                          return (
                            <tr
                              ...
                              className={`cursor-pointer ${selected ? 'bg-brand-light' : 'hover:bg-white'} ${rIsPar ? 'font-semibold' : ''}`}
                            >
                              <td className="py-1.5">
                                <input type="checkbox" checked={selected} readOnly className="w-3 h-3 rounded border-gray-300 text-brand" />
                              </td>
        ...
        <>
                ...
                {(showAll ? results : results.slice(0, 20)).map((r, i) => {
                  ...
                  return (
                    <tr
                      key={i}
                      onClick={() => toggleRate(r)}
                      className={`cursor-pointer transition-colors ${selected ? 'bg-brand-light' : 'hover:bg-gray-50'} ${isPar ? 'font-medium' : ''}`}
                    >
                      <td className="px-2 py-2 text-center">
                        <input type="checkbox" checked={selected} readOnly className="w-3.5 h-3.5 rounded border-gray-300 text-brand" />
                      </td>
          ...
          {results.length > 20 && !showAll && (
            <div className="px-6 py-2 border-t border-gray-100">
              <button onClick={() => setShowAll(true)} className="text-xs text-brand hover:underline">
      ...
      {selectedRates.length > 0 && (
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-900 rounded-b-xl flex items-center justify-between">
          ...
          <button
            onClick={onNext}
            className="px-6 py-2 bg-brand text-white rounded-lg text-sm font-bold hover:bg-brand-dark transition-colors tracking-wide"
          >
            NEXT: FEES & PREVIEW
          </button>
        </div>
