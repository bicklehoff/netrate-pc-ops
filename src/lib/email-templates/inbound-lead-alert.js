...
export function inboundLeadAlertTemplate({
  ...
  const phoneHtml = phone
    ? `<a href="tel:${escapeHtml(phone)}" style="color:#2E6BA8;text-decoration:none;font-weight:600;">${escapeHtml(phone)}</a>`
    : null;
  const emailHtml = `<a href="mailto:${escapeHtml(email)}" style="color:#2E6BA8;text-decoration:none;">${escapeHtml(email)}</a>`;
...
<body style="margin:0;padding:24px;background:#f9fafb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e9e7e0;border-radius:12px;overflow:hidden;">
    <tr>
      <td style="padding:20px 24px;background:#2E6BA8;color:#ffffff;">
        <div style="font-size:12px;letter-spacing:0.1em;text-transform:uppercase;color:#FFC220;font-weight:700;">New Inbound Lead</div>
        <div style="font-size:22px;font-weight:700;margin-top:4px;color:#ffffff;">${escapeHtml(name)}</div>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td style="padding:0 24px 24px;">
        <a href="${escapeHtml(contactLink)}" style="display:inline-block;padding:12px 20px;background:#059669;color:#ffffff;text-decoration:none;border-radius:6px;font-size:14px;font-weight:700;">Open in CoreCRM →</a>
      </td>
    </tr>
    <tr>
