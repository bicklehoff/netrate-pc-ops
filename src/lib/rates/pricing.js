...
// Lender-specific LLPA data (loaded from parsed rate sheet XLSX files)
// Falls back to GSE standard when lender-specific data not available
let _lenderLlpaCache = {};
function loadLenderLLPAs(lenderId) {
  if (_lenderLlpaCache[lenderId] !== undefined) return _lenderLlpaCache[lenderId];
  try {
    // Dynamic require — these are JSON files generated by scripts/extract-lender-llpas.js
    const data = require(`@/data/lender-llpas/${lenderId}.json`);
    _lenderLlpaCache[lenderId] = data;
    return data;
  } catch {
    _lenderLlpaCache[lenderId] = null;
    return null;
  }
}
